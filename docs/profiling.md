...
Vidur profiling scripts support `--gpu_vendor {auto,nvidia,amd}` for telemetry capture. On ROCm hosts:

- Telemetry is collected from `amd-smi --json` when `--gpu_vendor amd` (or `auto` with only `amd-smi` on `PATH`).
- Profiling scripts set both `CUDA_VISIBLE_DEVICES` and `HIP_VISIBLE_DEVICES` so Ray worker binding works for CUDA and ROCm.
- If `flashinfer` is selected on AMD, Vidur automatically falls back to a ROCm-compatible attention backend (prefers `triton`, then `xformers`, `flash_attn`, `sdpa`, `torch_sdpa`) when available.
...
For a reproducible ROCm environment and accuracy-validation workflow against real vLLM-ROCm runs, see [docs/rocm_validation.md](rocm_validation.md).
